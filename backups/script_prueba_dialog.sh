...
establecer_vlan() {
  # Detectar interfaces (excluimos loopback y capturamos nombre)
  mapfile -t interfaces < <(ip -br a | awk 'NR>1 {print $1}' | grep -v lo || true)

  if [[ ${#interfaces[@]} -eq 0 ]]; then
    dialog --title "Interfaces" --msgbox "No se encontraron interfaces válidas." 6 50
    return 1
  fi

  # Construir array para --menu: etiquetas numeradas (1,2,3...) y descripción = interfaz
  local menu_args=()
  for i in "${!interfaces[@]}"; do
    idx=$((i + 1))
    menu_args+=("$idx" "${interfaces[$i]}")
  done

  # Pedir cuántas interfaces desea configurar
  if ! num_ifaces=$(input_box "Configurar interfaces" "¿Cuántas interfaces quieres configurar?" "1"); then
    dialog --title "Cancelar" --msgbox "Operación cancelada." 6 40
    return 1
  fi

  if ! [[ "$num_ifaces" =~ ^[0-9]+$ ]] || ((num_ifaces < 1)); then
    dialog --title "Error" --msgbox "Número inválido." 6 40
    return 1
  fi

  declare -A iface_config
  for ((n = 1; n <= num_ifaces; n++)); do
    # Seleccionar interfaz mediante un menú con índices numéricos
    if ! sel=$(dialog --title "Seleccionar interfaz #$n" --menu "Elige la interfaz (usa número o flechas):" 0 0 0 "${menu_args[@]}" 3>&1 1>&2 2>&3); then
      dialog --title "Cancelar" --msgbox "Selección cancelada." 6 40
      return 1
    fi

    # Convertir sel (índice) a nombre de interfaz
    sel_index=$((sel - 1))
    if ((sel_index < 0 || sel_index >= ${#interfaces[@]})); then
      dialog --title "Error" --msgbox "Índice inválido." 6 40
      return 1
    fi
    base_iface="${interfaces[$sel_index]}"

    # Pedir IP/CIDR para la interfaz seleccionada
    if ! ip_iface=$(input_box "IP para $base_iface" "Dirección IP/CIDR para $base_iface (ej: 192.168.1.10/24):" ""); then
      dialog --title "Cancelar" --msgbox "Operación cancelada." 6 40
      return 1
    fi

    if ! [[ "$ip_iface" =~ ^([0-9]{1,3}\.){3}[0-9]{1,3}/[0-9]{1,2}$ ]]; then
      dialog --title "Error" --msgbox "IP inválida. Formato esperado: x.x.x.x/NN" 6 50
      return 1
    fi
  ...
  dialog --title "Resumen" --textbox "$TMP" 15 60

  if yesno_box "Confirmación" "¿Estás de acuerdo con esta configuración?"; then
    if [[ "$OS" == "Ubuntu" ]]; then
      generar_netplan "$NETPLAN_FILE" iface_config
    elif [[ "$OS" == "Debian" ]]; then
      generar_interfaces_debian "$DEBIAN_FILE" iface_config
    else
      dialog --title "Error" --msgbox "Sistema operativo no soportado." 6 50
      return 1
    fi
  else
    dialog --title "Reiniciar" --msgbox "Reiniciando configuración..." 5 40
    establecer_vlan # reiniciamos la función (puedes cambiar comportamiento si no deseas bucle)
  fi
}
