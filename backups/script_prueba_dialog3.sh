...
establecer_vlan() {
  ...
  # --- Preparar filas para --form ---
  form_args=()
  row=1
  for i in "${!interfaces[@]}"; do
    label="$((i + 1))) ${interfaces[$i]}"
    # Cada línea de label, valor vacío, coordenadas (fila,col), longitud label, longitud input
    form_args+=("$label" $row 1 "" $row 30 20 0)
    ((row++))
  done

  # Añadir campo para número de interfaces a configurar al final
  form_args+=("Número de interfaces a configurar:" $row 1 "" $row 30 5 0)

  # --- Mostrar el form ---
  form_result=$(dialog --title "Configurar VLANs" --form "Interfaces detectadas:" 20 60 $((row + 1)) \
    "${form_args[@]}" 3>&1 1>&2 2>&3)

  ret=$?
  [[ $ret -ne 0 ]] && rm -f "$TMP" && return 1 # Cancelar

  # El resultado viene todo en una línea separada por saltos de línea
  # Extraer la última línea como número de interfaces
  num_ifaces=$(echo "$form_result" | tail -n1)

  if ! [[ "$num_ifaces" =~ ^[0-9]+$ ]]; then
    dialog --title "Error" --msgbox "Número de interfaces inválido." 7 50
    rm -f "$TMP"
    return 1
  fi
  ...
  for ((n = 1; n <= num_ifaces; n++)); do
    ...
    # Construir menú de selección de interfaz
    menu_items=()
    for i in "${!interfaces[@]}"; do
    ...
    ret=$?
    [[ $ret -ne 0 ]] && rm -f "$TMP" && return 1

    iface_index=$((idx - 1))
    base_iface="${interfaces[$iface_index]}"

    # Evitar seleccionar la misma interfaz dos veces
  ...
  # --- Confirmación ---
  dialog --title "Confirmar" --yesno "¿Aplicar esta configuración?" 8 60
  ret=$?
  [[ $ret -ne 0 ]] && rm -f "$TMP" && return 1

  # --- Aplicar según sistema operativo ---
  if [[ "$OS" == "Ubuntu" ]]; then
