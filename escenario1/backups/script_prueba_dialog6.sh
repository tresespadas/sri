...
configurar_hosts() {
  ...
  dialog --title "Hosts" --infobox "Configurando /etc/hosts..." 5 50
  sleep 1
  ...
  #
  # 1) Preguntar cuántas líneas agregar
  #
  while true; do
  ...
  for ((i = 1; i <= num_hosts; i++)); do
    ...
    while true; do
      ip=$(dialog --title "Entrada #$i" \
        --inputbox "Introduce la IP (ejemplo: 192.168.1.50):" \
        10 60 \
        3>&1 1>&2 2>&3)
    ...
    fqdn=$(dialog --title "Entrada #$i" \
      --inputbox "Introduce el FQDN (ejemplo: node01.example.com):" \
      10 60 \
  ...
  #
  # 3) Preguntar explícitamente qué hostname quiere el usuario
  #
  nuevo_hostname=$(dialog --title "Hostname del sistema" \
    --inputbox "Introduce el hostname COMPLETO (FQDN) que quieres asignar a la máquina:\n\nEjemplo: server01.example.com" \
    12 70 \
    3>&1 1>&2 2>&3) || return 1

  if [[ -z "$nuevo_hostname" ]]; then
    dialog --title "Error" --msgbox "El hostname no puede estar vacío." 7 50
  else
    if sudo hostnamectl set-hostname "$nuevo_hostname"; then
      dialog --title "Hostname" --msgbox "Hostname cambiado correctamente a:\n\n$nuevo_hostname" 10 60
    else
      dialog --title "Error" --msgbox "No se pudo cambiar el hostname." 7 50
    fi
  fi

  #
  # 4) Mostrar contenido actualizado
  #
  cp "$hosts_file" "$TMP"
